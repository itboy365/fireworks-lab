...
function init() {
    ...
    // shell type - 修改为显示中文，但保留英文value值
    let options = '';
    shellNames.forEach(opt => {
        const chineseName = shellChineseNames[opt] || opt;
        options += `<option value="${opt}">${chineseName}</option>`;
    });
